...
proc flatUnitHelp(T:typedesc):static string{.compileTime.}=
  ...
  when T is Unit:
    ...
    "*"
proc flatUnitHelpIn(T:typedesc[Unit]):typedesc{.compileTime.}=
  ## 得到复合Unit的内部数值类型
  when T.T is Unit:
    flatUnitHelpIn T.T
  else:
...
when isMainModule:
  ...
  const g = 9.80665{meter/second^2}     # Standard gravity
  let height = 0.1{kilo*meter}
  proc getTime1[T;U:static string](h:Unit[T,U],g:wisUSi(T,U^0.5*"/s"^2*U^(1/2))):USi[T,"s"]=sqrt T(2)*h.wisSito(g.unit*"second"^2)/g
  # Unit是本体,右侧为字符串约束
  # USi是concept,根据Si系统匹配
  # wisUSi是宏,生成concept,用来处理复杂的泛型的乘除幂操作
  # wisSiTo是有关siTo的宏，可以方便处理siTo下的乘除幂操作
  proc getTime2[T](h:USi[T,"m"],g:USi[T,"m/s^2"]):USi[T,"s"]=(T(2)*h/g).sqrt.siTo "s"
  # 另一种函数推荐生成方法

  let fallTime = getTime1(height,g)
  assert(fallTime==getTime2(height,g))

  echo "Fall time: ", fallTime          # 4.5160075575178755 second

  let impactEnergy = height * 75.0{kilogram} * g
  ...
  echo "Material stress: ", stress.siTo "kPa"  # 100.0 kpa
